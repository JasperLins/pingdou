package cn.pixel.pingdou.module.infra.service.codegen.inner;
...
import cn.pixel.pingdou.framework.excel.core.annotations.DictFormat;
import cn.pixel.pingdou.framework.excel.core.convert.DictConvert;
import cn.pixel.pingdou.framework.excel.core.util.ExcelUtils;
import cn.pixel.pingdou.framework.mybatis.core.dataobject.BaseDO;
import cn.pixel.pingdou.framework.mybatis.core.mapper.BaseMapperX;
import cn.pixel.pingdou.framework.mybatis.core.query.LambdaQueryWrapperX;
import cn.pixel.pingdou.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import cn.pixel.pingdou.module.infra.dal.dataobject.codegen.CodegenTableDO;
import cn.pixel.pingdou.module.infra.enums.codegen.CodegenFrontTypeEnum;
import cn.pixel.pingdou.module.infra.enums.codegen.CodegenSceneEnum;
import cn.pixel.pingdou.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import cn.pixel.pingdou.module.infra.enums.codegen.CodegenVOTypeEnum;
import cn.pixel.pingdou.module.infra.framework.codegen.config.CodegenProperties;
...
import static cn.pixel.pingdou.module.infra.enums.ErrorCodeConstants.CODEGEN_MASTER_TABLE_FIELD_DUPLICATE;
import static cn.pixel.pingdou.module.infra.enums.ErrorCodeConstants.CODEGEN_MASTER_TABLE_NAME_DUPLICATE;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 pingdou-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 pingdou-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("cn.pixel.pingdou.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/pingdou-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/pingdou-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "pingdou-module-${table.moduleName}/" + // 顶级模块
                "pingdou-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "pingdou-module-${table.moduleName}/" + // 顶级模块
                "pingdou-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "pingdou-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "pingdou-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "pingdou-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "pingdou-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
