package cn.pixel.pingdou.module.iot.core.messagebus.config;

import cn.pixel.pingdou.framework.mq.redis.core.RedisMQTemplate;
import cn.pixel.pingdou.framework.mq.redis.core.job.RedisPendingMessageResendJob;
import cn.pixel.pingdou.framework.mq.redis.core.job.RedisStreamMessageCleanupJob;
import cn.pixel.pingdou.framework.mq.redis.core.stream.AbstractRedisStreamMessage;
import cn.pixel.pingdou.framework.mq.redis.core.stream.AbstractRedisStreamMessageListener;
import cn.pixel.pingdou.module.iot.core.messagebus.core.IotMessageBus;
import cn.pixel.pingdou.module.iot.core.messagebus.core.kafka.IotKafkaMessageBus;
import cn.pixel.pingdou.module.iot.core.messagebus.core.local.IotLocalMessageBus;
import cn.pixel.pingdou.module.iot.core.messagebus.core.rabbitmq.IotRabbitMQMessageBus;
import cn.pixel.pingdou.module.iot.core.messagebus.core.redis.IotRedisMessageBus;
import cn.pixel.pingdou.module.iot.core.messagebus.core.rocketmq.IotRocketMQMessageBus;
...
public class IotMessageBusAutoConfiguration {
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "pingdou.iot.message-bus", name = "type", havingValue = "local", matchIfMissing = true)
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "pingdou.iot.message-bus", name = "type", havingValue = "rocketmq")
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "pingdou.iot.message-bus", name = "type", havingValue = "kafka")
    ...
    /**
     * 特殊：由于 PingdouRedisMQConsumerAutoConfiguration 关于 Redis stream 的消费是动态注册，所以这里只能拷贝相关的逻辑！！！
     *
     * @see cn.pixel.pingdou.framework.mq.redis.config.PingdouRedisMQConsumerAutoConfiguration
     */
    @Configuration
    @ConditionalOnProperty(prefix = "pingdou.iot.message-bus", name = "type", havingValue = "redis")
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "pingdou.iot.message-bus", name = "type", havingValue = "rabbitmq")
